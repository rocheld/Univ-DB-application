        ...
        <tr>
            ...
            <%
                ...
                if (action != null && action.equals("Search")) {
                    ...
                    PreparedStatement pstmt2 = conn.prepareStatement(
                            "select w.* from nonweeklyschedule w " +
            ...
            <%
                    ...
                    if (action != null && action.equals("winsert")) {
                        ...
                        PreparedStatement stmt = conn.prepareStatement(
                            "insert into public.nonweeklyschedule (section_id,sched_type,start_time) values (?,?,?)");                        

                        PreparedStatement up = conn.prepareStatement (
                            "Update public.nonweeklyschedule set end_time = start_time + '1 hour' where section_id = ? AND sched_type = ?AND start_time = ?"
                        );

                        stmt.setString(1,request.getParameter("SID"));
                        stmt.setString(2,request.getParameter("type_select"));
                        stmt.setTimestamp(3, Timestamp.valueOf(request.getParameter("START")));

                        up.setString(1,request.getParameter("SID"));
                        up.setString(2,request.getParameter("type_select"));
                        up.setTimestamp(3, Timestamp.valueOf(request.getParameter("START")));
            ...
            <%
                    ...
                    if (action != null && action.equals("update")) {
                        ...
                        PreparedStatement stmt = conn.prepareStatement(
                            "Update public.nonweeklyschedule set section_id = ?, sched_type = ? , start_time = ? where index = ?"
                        );
                        PreparedStatement up = conn.prepareStatement(
                            "Update public.nonweeklyschedule set end_time = start_time + '1 hour' where index = ?"
                        );
                        stmt.setString(1,request.getParameter("SID"));
            ...
            <%
                    ...
                    if (action != null && action.equals("wdelete")) {
                        ...
                        PreparedStatement stmt = conn.prepareStatement(
                            "DELETE FROM nonweeklyschedule WHERE index = ? ");
            ...
            <!-- -------------------- HTML SELECT ----------------- -->
                ...
                <table border="1">
                    <th>Select a Section
                    <form action="section_schedule_nw.jsp" method="POST">
                ...
                <table border="1">
                    ...
                    <tr>
                    <form action="section_schedule_nw.jsp" method="get">
                            <input type="hidden" value="winsert" name="action">
                            <th><input value="<%=sid%>" name="SID" size="10" readonly> </th>
                            <th> <select name="type_select"><option value="RE">RE</option><option value="FI">FI</option></select> </th>
