    ...
    <table border="10">
        <tr>
            ...
            <%
                ...
                if (action != null && action.equals("Search")) {
                    ...
                    PreparedStatement pstmt5 = conn.prepareStatement (
                        "CREATE TABLE IF NOT EXISTS temp AS ( SELECT e.pid FROM courseEnrollment e INNER JOIN section s ON s.section_id = e.sid AND s.section_id = '1' INNER JOIN classes c ON s.class_id = c.index WHERE class_quarter = 'sp' AND class_year = '2020')"

                    );
                    rs = pstmt5.executeQuery();
                    // Commit transaction

                    PreparedStatement pstmt6 = conn.PreparedStatement(
                    "SELECT * FROM time_list t EXCEPT SELECT tt.t1, tt.t2 FROM time_list tt, (SELECT qs.start_time, qs.end_time FROM courseEnrollment e INNER JOIN quarter_schedule qs ON e.sid = qs.sid WHERE EXISTS ( SELECT * FROM temp t WHERE e.pid = t.pid)) AS qs2 WHERE (tt.t1, tt.t2) overlaps(qs2.start_time, qs2.end_time)"
                );

                    rr = pstmt6.executeQuery();
            ...
            %>
                    <tr>
                            <td>
                                <input value="<%= rs.getString("t1") %>" 
                                    name="Student ID" size="10">
                            </td>
    
                            <td>
                                <input value="<%= rs.getString("t2") %>" 
